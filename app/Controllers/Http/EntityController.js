...
class EntityController {
    async show(request, response) {
        const entities = await Database
            .raw(`select id, modelo->>'RAZON_SOCIAL' as razon_social, modelo->>'NUMERO_DOCUMENTO' as numero_documento from  nbc_entidades order by modelo->>'RAZON_SOCIAL'`)
    ...
    async showContractById({ request, response: { id } }) {
        const entities = await Database                                                                                                                                                                                                                                            //REMPLAZAR 54 POR EL ID QUE RECIBE
            .raw(`select nbc_sedes_contrato.id, nbc_sedes_contrato.modelo->>'DESCRIPCION_SEDE' as descripcion_sede from nbc_sedes_contrato, nbc_contratos where nbc_contratos.id = (nbc_sedes_contrato.modelo->>'ID_CONTRATO')::integer and (nbc_contratos.modelo->>'PRESTADOR_ID')::integer = 54 order by nbc_sedes_contrato.modelo->>'DESCRIPCION_SEDE'`)
        return entities.rows
    }

    async showSedeById({ request, response: { id } }) {
        const entities = await Database 
        // remplazar el 1717 en la linea 60                                                                                                                                                                                                                                        //    REMPLAZAR 54 POR EL ID QUE RECIBE
            .raw(`with    base_data_1 as (
