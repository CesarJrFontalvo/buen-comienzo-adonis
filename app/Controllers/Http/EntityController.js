...
class EntityController {
    ...
    async showListSedeById({ request, response}) {
        const entities = await Database                                                                                                                                                                                                                                            
            .raw(`select nbc_sedes_contrato.id, nbc_sedes_contrato.modelo->>'DESCRIPCION_SEDE' as descripcion_sede from nbc_sedes_contrato, nbc_contratos where nbc_contratos.id = (nbc_sedes_contrato.modelo->>'ID_CONTRATO')::integer and (nbc_contratos.modelo->>'PRESTADOR_ID')::integer = ? order by nbc_sedes_contrato.modelo->>'DESCRIPCION_SEDE'`,[request.params.id])
        return entities.rows
    }
    ...
    async showParticipantDetailById({ request, response}) {
        ...
        const entities = await Database 
            .raw(`with    my_parameters as (
                select  ${id} as id_sede
            ),
            base_data_1 as (
    ...
    async showDetailSedeById({ request, response}) {
        const entities = await Database                                                                                                                                                                                                                                            
            .raw(`select  nbc_sedes.modelo->>'NOMBRE_SEDE' as nombre_sede,
            nbc_sedes.modelo->'UBICACION'->>'BARRIO' as id_barrio_sede,
            nbc_sedes.modelo->'UBICACION'->>'NOMBRE_BARRIO' as barrio_sede,
            nbc_sedes.modelo->'UBICACION'->>'DIRECCION' as direccion_sede,
            nbc_sedes.modelo->>'TELEFONO' as telefono_sede,
    ...
    and     nbc_contratos.id   = (nbc_sedes_contrato.modelo->>'ID_CONTRATO')::integer
    and     nbc_modalidades.id = (nbc_contratos.modelo->>'MODALIDAD_ID')::integer
    and     nbc_sedes.id       = ?`,[request.params.id])
        return entities.rows
    }
}
